...
export default function ReportsPage() {
  ...
  const [isRunningReport, setIsRunningReport] = useState<number | null>(null);
  const [isDownloading, setIsDownloading] = useState<string | null>(null);

  const handleRunReport = async (reportId: number, reportName: string) => {
    setIsRunningReport(reportId);
    addToast(`Generating ${reportName}...`, "info");
    
    await new Promise((resolve) => setTimeout(resolve, 2500));
    
    setIsRunningReport(null);
    addToast(`${reportName} generated successfully!`, "success");
  };

  const handleDownloadReport = async (reportId: number, reportName: string, format: string) => {
    setIsDownloading(`report-${reportId}`);
    addToast(`Preparing ${reportName} for download...`, "info");
    
    await new Promise((resolve) => setTimeout(resolve, 1500));
    
    // Create demo report content
    const reportContent = `
${reportName}
...
IT: 95% completion, 88% avg score
Finance: 82% completion, 85% avg score  
HR: 78% completion, 84% avg score
Sales: 70% completion, 79% avg score
Operations: 68% completion, 81% avg score

=== RISK ASSESSMENT ===
Low Risk: 186 employees
Medium Risk: 45 employees
High Risk: 16 employees

Generated by CyberShield Training Platform
    `.trim();
    ...
    addToast(`${reportName} downloaded!`, "success");
  };
  ...
  const handleDownloadExport = async (fileName: string) => {
    ...
    const content = `Demo export file: ${fileName}\nGenerated: ${new Date().toISOString()}`;
    const blob = new Blob([content], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = fileName.replace(/\.(pdf|xlsx|csv)$/, ".txt");
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  const handleShareReport = async (fileName: string) => {
    try {
      await navigator.clipboard.writeText(
        `Report: ${fileName}\nShared from CyberShield Training Platform\nhttps://cybershield.app/reports/${fileName}`
      );
  ...
  const handleUseTemplate = (templateName: string) => {
    addToast(`Loading "${templateName}" template...`, "info");
    setTimeout(() => {
      addToast(`Template loaded. Configure your report settings.`, "success");
    }, 1000);
  };

  const handleCreateCustomReport = () => {
    addToast("Opening custom report builder...", "info");
  };

  const handleNewReport = () => {
    setActiveTab("templates");
    addToast("Select a template to create a new report", "info");
  };
        ...
        <div className="px-4 py-6 lg:px-8 lg:py-8">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-3">
              <Button variant="outline" className="border-border" onClick={handleFilter}>
                <Filter className="mr-2 h-4 w-4" />
                Filter
              </Button>
              <Button className="bg-primary text-primary-foreground hover:bg-primary/90" onClick={handleNewReport}>
                          ...
                          <td className="p-4">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <Button
                                ...
                                disabled={isDownloading === `report-${report.id}`}
                              >
                              ...
                              <Button
                                ...
                                size="sm"
                                className="text-muted-foreground hover:text-primary"
                                onClick={() => addToast(`Editing ${report.name}...`, "info")}
                              >
          ...
          {activeTab === "templates" && (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              ...
              <Card 
                className="border-border bg-card border-dashed hover:border-primary/30 transition-colors cursor-pointer"
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          onClick={() => handleDownloadExport(file.name)}
                        ...
                        <Button
                          ...
                          size="sm"
                          className="text-muted-foreground hover:text-primary"
                          onClick={() => handleShareReport(file.name)}
                        >
